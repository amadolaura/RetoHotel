...
public class HabitacionesController {
    ...
    @GetMapping("/{id}")
    public Optional<Room> getHabitaciones(@PathVariable("id") int id){
        return habitacionesService.getHabitaciones(id);
    }

    @PostMapping("/save")
    @ResponseStatus(HttpStatus.CREATED)
    public Room save(@RequestBody Room h){
        return habitacionesService.save(h);
    }
